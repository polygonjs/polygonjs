...
export function testcoreArrayUtils(qUnit: QUnit) {
	...
	qUnit.test('ArrayUtils.compact', (assert) => {
		assert.deepEqual(ArrayUtils.compact([7, null, undefined, 0, null, 3, 7], _numbersOrNull), [7, 0, 3, 7]);
	});
	...
	qUnit.test('ArrayUtils.union', (assert) => {
		assert.deepEqual(ArrayUtils.union([0, 1, 2], [2, 3, 4], _numbers), [0, 1, 2, 3, 4]);
	});
	...
	qUnit.test('ArrayUtils.range', (assert) => {
		assert.deepEqual(rangeWithEnd(5), [0, 1, 2, 3, 4]);

		assert.deepEqual(range(1, 5, 1, _numbers), [1, 2, 3, 4]);

		assert.deepEqual(ArrayUtils.range(1, 5, 2, _numbers), [1, 3]);

		assert.deepEqual(ArrayUtils.range(7, 10, 1, _numbers), [7, 8, 9]);
	});
}
