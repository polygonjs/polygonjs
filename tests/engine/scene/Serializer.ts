...
export function testenginesceneSerializer(qUnit: QUnit) {
	qUnit.test('scene save simple', async (assert) => {
		...
		await saveAndLoadScene(scene, async (scene2) => {
			...
			const newGeo1 = scene2.node(geo1.path())! as GeoObjNode;
			assert.ok(newGeo1.p.r.y.hasExpression());
			assert.equal(newGeo1.p.r.y.rawInput(), '$F+20');
			assert.equal(newGeo1.p.r.y.expressionController?.expression(), '$F+20');
			scene2.setFrame(12);
			await newGeo1.p.r.y.compute();
			assert.equal(newGeo1.p.r.y.value, 32);
			assert.deepEqual(newGeo1.p.r.value.toArray(), [0, 32, 0]);
		});
	});
}
