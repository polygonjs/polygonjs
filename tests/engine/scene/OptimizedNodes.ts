...
export function testenginesceneOptimizedNodes(qUnit: QUnit) {
	...
	qUnit.test('scene can be imported with a 2 optimized nodes plugged into each other', async (assert) => {
		...
		Poly.setPlayerMode(true);
		const scene_player = await SceneJsonImporter.loadData(data);
		assert.equal(scene_player.graph.nextId(), scene_no_player.graph.nextId() - 43, '-43');
		assert.equal(
			scene_player.nodesController.allNodes().length,
			scene_no_player.nodesController.allNodes().length - 1
		);
