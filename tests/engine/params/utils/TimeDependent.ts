...
export function testengineparamsutilsTimeDependent(qUnit: QUnit) {
	...
	qUnit.test(
		'a param sets its node to timedependent and a scene time change sets the node to dirty',
		async (assert) => {
	...
	qUnit.test('a node with 2 params can be time dependent', async (assert) => {
		...
		assert.equal(ry.graphAllPredecessors().length, 0, 'ry should not have any predecessors');
		assert.equal((time_graph_node.graphSuccessors() || []).length, 1);
		assert.deepEqual(
			(time_graph_node.graphSuccessors() || []).map((n) => n.graphNodeId()).sort(),
