...
QUnit.test('color keep their default and also for their components', async (assert) => {
	...
	assert.deepEqual(param.value_pre_conversion.toArray(), [1, 1, 1]);
	assert.equal(param.value_pre_conversion.r, 1);
	assert.equal(param.value_pre_conversion.g, 1);
	...
	// change only one component
	param.r.set(0.4);
	assert.deepEqual(param.value_pre_conversion.toArray(), [0.4, 1, 1]);
