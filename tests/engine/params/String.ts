...
export function testengineparamsString(qUnit: QUnit) {
	...
	qUnit.test('a string param using single quotes in its expression is sanitized correctly', async (assert) => {
		...
		assert.equal(_points[0].attribValue('html'), "<div style='will-change: transform, opacity;'>0</div>");
		assert.equal(_points[12].attribValue('html'), "<div style='will-change: transform, opacity;'>12</div>");
	});
