QUnit.test('gl if_then has child subnet_output disconnect if its own input is', async (assert) => {
	...
	assert.equal(if_then1.io.outputs.named_output_connection_points, 0);
	...
	assert.equal(subnet_input1.io.outputs.named_output_connection_points.length, 0);
	...
	assert.equal(subnet_output1.io.inputs.named_input_connection_points.length, 1);
	assert.equal(if_then1.io.outputs.named_output_connection_points.length, 0);
