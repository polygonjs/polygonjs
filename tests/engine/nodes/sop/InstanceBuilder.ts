...
export function testenginenodessopInstanceBuilder(qUnit: QUnit) {
	...
	function _prepareForSave(instanceBuilder1: InstanceBuilderSopNode) {
		const globals = instanceBuilder1.createNode('globals');
		const output = instanceBuilder1.createNode('output');
		const add1 = instanceBuilder1.createNode('add');
	...
	qUnit.test('sop/instanceBuilder persisted config is saved after scene play', async (assert) => {
		const scene = window.scene;
		...
		instanceBuilder1.setInput(0, box1);
		instanceBuilder1.flags.display.set(true);

		_prepareForSave(instanceBuilder1);
		...
		await RendererUtils.withViewer({cameraNode: perspective_camera1}, async (args) => {
			...
			assert.deepEqual(functionNodeNames, [instanceBuilder1.path()], 'actor is saved');
		});
		RendererUtils.dispose();
	});
	qUnit.test('sop/instanceBuilder persisted config is saved without requiring scene play', async (assert) => {
		const scene = window.scene;
		const geo1 = window.geo1;
		const box1 = geo1.createNode('box');
		const instanceBuilder1 = geo1.createNode('instanceBuilder');
	...
	qUnit.test(
		'sop/instanceBuilder persisted config is saved without requiring scene play nor display flag',
		async (assert) => {
			const scene = window.scene;
			const geo1 = window.geo1;
			const box1 = geo1.createNode('box');
