...
export function testenginenodessopActorPoint(qUnit: QUnit) {
	...
	qUnit.test('sop/actorPoint persisted config is saved after scene play', async (assert) => {
		const scene = window.scene;
		const perspective_camera1 = window.perspective_camera1;
		const geo1 = window.geo1;
		const box1 = geo1.createNode('box');
		...
		const onTick1 = actorPoint1.createNode('onTick');
		const setPointPosition = actorPoint1.createNode('setPointPosition');
		setPointPosition.setInput(0, onTick1);
		...
		await RendererUtils.withViewer({cameraNode: perspective_camera1}, async (args) => {
			...
			assert.deepEqual(functionNodeNames, [actorPoint1.path()], 'actor is saved');
		});
		RendererUtils.dispose();
	});
	qUnit.test('sop/actorPoint persisted config is saved without requiring scene play', async (assert) => {
		const scene = window.scene;
		// const perspective_camera1 = window.perspective_camera1;
		const geo1 = window.geo1;
		const box1 = geo1.createNode('box');
