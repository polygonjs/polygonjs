...
export function testenginenodessopSDFBuilder(qUnit: QUnit) {
	...
	function _prepareForSave(SDFBuilder1: SDFBuilderSopNode) {
		const globals = SDFBuilder1.createNode('globals');
		const output = SDFBuilder1.createNode('output');
		const SDFSphere1 = SDFBuilder1.createNode('SDFSphere');

		output.setInput('d', SDFSphere1);

		return {globals};
	}

	qUnit.test('sop/SDFBuilder persisted config is saved after scene play', async (assert) => {
		const scene = window.scene;
		...
		SDFBuilder1.setInput(0, box1);
		SDFBuilder1.flags.display.set(true);

		_prepareForSave(SDFBuilder1);
		...
		await RendererUtils.withViewer({cameraNode: perspective_camera1}, async (args) => {
			...
			assert.deepEqual(functionNodeNames, [SDFBuilder1.path()], 'actor is saved');
		});
		RendererUtils.dispose();
	});
	qUnit.test('sop/SDFBuilder persisted config is saved without requiring scene play', async (assert) => {
		const scene = window.scene;
		const geo1 = window.geo1;
		const box1 = geo1.createNode('box');
		const SDFBuilder1 = geo1.createNode('SDFBuilder');
	...
	qUnit.test(
		'sop/SDFBuilder persisted config is saved without requiring scene play nor display flag',
		async (assert) => {
			const scene = window.scene;
			const geo1 = window.geo1;
			const box1 = geo1.createNode('box');
