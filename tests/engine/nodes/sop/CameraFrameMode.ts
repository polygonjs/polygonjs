...
qUnit.test('sop/cameraFrameMode simple', async (assert) => {
	...
	assert.equal(
		CoreObject.attribValue(objects[0], CameraAttribute.FRAME_MODE),
...
qUnit.test('sop/cameraFrameMode applyToChildren', async (assert) => {
	...
	assert.equal(CoreObject.attribValue(objects[0], CameraAttribute.FRAME_MODE_EXPECTED_ASPECT_RATIO), 16 / 9);
	assert.equal(
		CoreObject.attribValue(objects[0].children[0], CameraAttribute.FRAME_MODE),
		CAMERA_FRAME_MODES.indexOf(CameraFrameMode.DEFAULT)
	);
	assert.equal(
		CoreObject.attribValue(objects[0].children[0], CameraAttribute.FRAME_MODE_EXPECTED_ASPECT_RATIO),
		16 / 9
	);
});
