...
export function testenginenodessopAttribCast(qUnit: QUnit) {
	qUnit.test('attribCast simple', async (assert) => {
		...
		assert.ok(index instanceof BufferAttribute, 'BufferAttribute');
		assert.ok(index instanceof Uint16BufferAttribute, 'Uint16BufferAttribute');
	});
}
