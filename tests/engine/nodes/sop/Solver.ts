...
export function testenginenodessopSolver(qUnit: QUnit) {
	...
	qUnit.test('sop/solver simple', async (assert) => {
		...
		scene.cooker.unblock();
		await computeSolver();
		assert.in_delta(size.z, 1.5, 0.1, 'bbox size x');
		assert.ok(!solver1.states.error.message(), 'no error');

		solver1.p.iterations.set(1);
		await computeSolver();
		assert.in_delta(size.z, 1.23, 0.01, 'bbox size x 2');
		assert.ok(!solver1.states.error.message(), 'no error');

		solver1.p.iterations.set(2);
		await computeSolver();
		assert.in_delta(size.z, 1.5, 0.01, 'bbox size x 3');
		assert.ok(!solver1.states.error.message(), 'no error');

		solver1.p.iterations.set(3);
		await computeSolver();
		assert.in_delta(size.z, 1.96, 0.01, 'bbox size x 4');
		assert.ok(!solver1.states.error.message(), 'no error');

		solver1.p.iterations.set(4);
		await computeSolver();
		assert.in_delta(size.z, 1.96, 0.01);
		assert.ok(!solver1.states.error.message());

		solver1.p.iterations.set(5);
		await computeSolver();
		assert.in_delta(size.z, 2.45, 0.01);
		assert.ok(!solver1.states.error.message());
	});
}
