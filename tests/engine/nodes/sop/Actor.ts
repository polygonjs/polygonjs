...
export function testenginenodessopActor(qUnit: QUnit) {
	...
	qUnit.test('sop/actor persisted config is saved after scene play', async (assert) => {
		const scene = window.scene;
		const perspective_camera1 = window.perspective_camera1;
		const geo1 = window.geo1;
		const box1 = geo1.createNode('box');
		...
		await RendererUtils.withViewer({cameraNode: perspective_camera1}, async (args) => {
			...
			assert.deepEqual(functionNodeNames, [actor1.path()], 'actor is saved');
		});
		RendererUtils.dispose();
	});
	qUnit.test('sop/actor persisted config is saved without requiring scene play', async (assert) => {
		const scene = window.scene;
		// const perspective_camera1 = window.perspective_camera1;
		const geo1 = window.geo1;
		const box1 = geo1.createNode('box');
	...
	qUnit.test('sop/actor persisted config is saved without requiring scene play nor display flag', async (assert) => {
		const scene = window.scene;
		// const perspective_camera1 = window.perspective_camera1;
		const geo1 = window.geo1;
		const box1 = geo1.createNode('box');
