...
export function testenginenodessopCopy(qUnit: QUnit) {
	...
	qUnit.test('sop/copy can handle expression inside a nodePath param such as sop/material', async (assert) => {
		async function runAsserts(
			_copyNode: CopySopNode,
			_imageNodes: ImageCopNode[],
			_materialNodes: MeshBasicMatNode[]
		) {
	...
	qUnit.test('sop/copy with no second input and srcGroup', async (assert) => {
		const geo1 = window.geo1;
		...
		copy1.setInput(0, box1);
		objectProperties.setInput(0, copy1);
		copy2.setInput(0, objectProperties);

		copy1.p.t.x.set(1);
		copy1.p.count.set(4);
		objectProperties.p.tname.set(true);
		objectProperties.p.name.set(`obj-\`@objnum\``);
		...
		assert.equal((await computeCopy(copy2)).length, 8);

		copy2.p.srcGroup.set('obj-1');
		assert.equal((await computeCopy(copy2)).length, 2);
		assert.deepEqual(
			(await computeCopy(copy2)).map((o) => o.name),
			['obj-1', 'obj-1'],
			'obj-1 2x'
		);

		copy2.p.srcGroup.set('obj-1 obj-2');
		assert.equal((await computeCopy(copy2)).length, 4);
		assert.deepEqual(
			(await computeCopy(copy2)).map((o) => o.name),
	...
	qUnit.test('sop/copy with second input and srcGroup', async (assert) => {
		const geo1 = window.geo1;
		...
		const copy1 = geo1.createNode('copy');
		const copy2 = geo1.createNode('copy');
		const copy3 = geo1.createNode('copy');
		...
		objectProperties1.p.name.set(`box-\`@objnum\``);

		// template
		copy2.setInput(0, plane1);
		objectProperties2.setInput(0, copy2);
		copy2.p.t.y.set(1);
		copy2.p.count.set(2);
		objectProperties2.p.tname.set(true);
		objectProperties2.p.name.set(`plane-\`@objnum\``);

		// copy
		copy3.setInput(0, objectProperties1);
		copy3.setInput(1, objectProperties2);
		// copy3.p.count.set(2);
		// copy3.p.t.y.set(1);
		async function computeCopy(copy: CopySopNode) {
			const container = await copy.compute();
		...
		assert.equal((await computeCopy(copy3)).length, 16);
		assert.deepEqual(
			(await computeCopy(copy3)).map((o) => o.name),
			[
				'box-0',
				'box-1',
				'box-0',
		...
		copy3.p.srcGroup.set('box-1');
		assert.equal((await computeCopy(copy3)).length, 8);
		assert.deepEqual(
			(await computeCopy(copy3)).map((o) => o.name),
		...
		copy3.p.templateGroup.set('plane-1');
		assert.equal((await computeCopy(copy3)).length, 4);
		assert.deepEqual(
			(await computeCopy(copy3)).map((o) => o.name),
			['box-1', 'box-1', 'box-1', 'box-1'],
			'src and template group'
		);
		copy3.p.srcGroup.set('');
		assert.equal((await computeCopy(copy3)).length, 8);
		assert.deepEqual(
			(await computeCopy(copy3)).map((o) => o.name),
			['box-0', 'box-1', 'box-0', 'box-1', 'box-0', 'box-1', 'box-0', 'box-1'],
			'template group only'
		);
	});
}
