...
export function testenginenodessopResample(qUnit: QUnit) {
	qUnit.test('resample a line', async (assert) => {
		...
		assert.equal(container.coreContent()!.pointsCount(), 101);
		...
		resample1.setCurveType(SplineCurveType.CENTRIPETAL);
		container = await resample1.compute();
		assert.equal(container.coreContent()!.pointsCount(), 22);
	...
	qUnit.test('resample a text', async (assert) => {
		...
		assert.equal(container.coreContent()!.pointsCount(), 738);
		...
		resample1.setCurveType(SplineCurveType.CENTRIPETAL);
		container = await resample1.compute();
		assert.equal(container.coreContent()!.pointsCount(), 505);
		...
		container = await resample1.compute();
		assert.equal(container.coreContent()!.pointsCount(), 2505);
		...
		resample1.setCurveType(SplineCurveType.CENTRIPETAL);
		container = await resample1.compute();
		assert.equal(container.coreContent()!.pointsCount(), 21);
		...
		container = await resample1.compute();
		assert.equal(container.coreContent()!.pointsCount(), 1235);

		resample1.p.segmentLength.set(0.04);
		container = await resample1.compute();
		assert.equal(container.coreContent()!.pointsCount(), 314);

		// also vary tension
		resample1.p.tension.set(0);
		container = await resample1.compute();
		assert.equal(container.coreContent()!.pointsCount(), 314, 'with tension 0');

		resample1.p.tension.set(0.01);
		container = await resample1.compute();
		assert.equal(container.coreContent()!.pointsCount(), 314, 'with tension 0.01');

		resample1.p.tension.set(0.5);
		container = await resample1.compute();
		assert.equal(container.coreContent()!.pointsCount(), 329);

		resample1.p.tension.set(1);
		container = await resample1.compute();
		assert.equal(container.coreContent()!.pointsCount(), 354);
	});
}
