...
export function testenginenodessopCameraWebXRARMarkerTracking(qUnit: QUnit) {
	qUnit.test('sop/cameraWebXRARMArkerTracking simple', async (assert) => {
		...
		assert.equal(ThreejsCoreObject.attribValue(objects[0], CameraAttribute.WEBXR_AR_MARKER_TRACKING), undefined);
		assert.equal(
	...
	qUnit.test('sop/cameraWebXRARMArkerTracking applyToChildren', async (assert) => {
		...
		assert.equal(ThreejsCoreObject.attribValue(objects[0], CameraAttribute.WEBXR_AR_MARKER_TRACKING), undefined);
		assert.equal(
			ThreejsCoreObject.attribValue(objects[0].children[0], CameraAttribute.WEBXR_AR_MARKER_TRACKING),
			undefined
		);
