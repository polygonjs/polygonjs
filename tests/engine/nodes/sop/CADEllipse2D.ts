...
qUnit.test('sop/CADEllipseD simple', async (assert) => {
	...
	assert.equal(((await computeTriangulate()).geometry.getAttribute('position') as BufferAttribute).array.length, 960);
	assert.in_delta(tmpBox.min.y, -1, 0.01);
	assert.in_delta(tmpSize.y, 2, 0.01);

	ellipse2D1.p.minorRadius.set(2);

	assert.equal(((await computeTriangulate()).geometry.getAttribute('position') as BufferAttribute).array.length, 948);
