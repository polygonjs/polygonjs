...
export function testenginenodessopCameraRenderScene(qUnit: QUnit) {
	qUnit.test('sop/cameraRenderScene simple', async (assert) => {
		...
		assert.equal(
			ThreejsCoreObject.attribValue(objects[0], CameraAttribute.RENDER_SCENE_NODE_ID),
			scene1.graphNodeId()
		);
	});
	qUnit.test('sop/cameraRenderScene applyToChildren', async (assert) => {
		...
		assert.equal(
			ThreejsCoreObject.attribValue(objects[0].children[0], CameraAttribute.RENDER_SCENE_NODE_ID),
			scene1.graphNodeId()
		);
	});
}
