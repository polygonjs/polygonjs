...
export function testenginenodessopAttribCopy(qUnit: QUnit) {
	qUnit.test('sop/attribCopy vertex latitude to position', async (assert) => {
		...
		assert.equal(array.length, container.coreContent()!.pointsCount() * 3, 'array is 3x the points count');
		...
		assert.equal(array.length, container.coreContent()!.pointsCount() * 3, 'array is 3x points_count');
		assert.equal(array[0], 0);
		assert.equal(array[3], 1);
		assert.equal(array[6], 2);
