...
export function testenginenodessopMapboxLayer(qUnit: QUnit) {
	...
	qUnit.test('sop/mapboxLayer simple', async (assert) => {
		...
		await RendererUtils.withViewer({viewer, mount: true}, async ({viewer, element}) => {
			...
			assert.equal(container1.coreContent()!.totalPointsCount(), 0, 'no points yet');
			...
			assert.more_than(container2.coreContent()!.totalPointsCount(), 1000, 'more than 1000 points created');
