...
export function testenginenodessopMetaball(qUnit: QUnit) {
	...
	qUnit.test('metaball simple', async (assert) => {
		...
		assert.equal(result.coreGroup.points(points).length, 64000);
		...
		result = await compute(metaball1);
		assert.equal(result.coreGroup.points(points).length, 8);

		metaball1.p.resolution.set(20);
		result = await compute(metaball1);
		assert.equal(result.coreGroup.points(points).length, 8000);
	});
