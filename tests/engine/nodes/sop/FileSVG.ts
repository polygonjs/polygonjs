...
import {objectsCount, objectsCountByType} from '../../../../src/engine/containers/utils/GeometryContainerUtils';
export function testenginenodessopFileSVG(qUnit: QUnit) {
	...
	qUnit.test('SOP svg with tiger', async (assert) => {
		...
		assert.equal(objectsCount(container), 466);
		assert.equal(container.coreContent()!.pointsCount(), 164050);
		assert.deepEqual(objectsCountByType(container), {Mesh: 466});
	...
	qUnit.test('SOP svg with wolf', async (assert) => {
		...
		const core_content = container.coreContent()!;
		assert.equal(objectsCount(container), 60);
		assert.equal(container.coreContent()!.pointsCount(), 43260);
		assert.deepEqual(objectsCountByType(container), {Mesh: 60});
