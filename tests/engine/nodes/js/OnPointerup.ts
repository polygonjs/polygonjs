...
import {triggerPointerupInMiddle} from '../../../helpers/EventsHelper';
import {RendererUtils} from '../../../helpers/RendererUtils';
import {CoreEventEmitter} from '../../../../src/core/event/CoreEventEmitter';
export function testenginenodesjsOnPointerup(qUnit: QUnit) {
	qUnit.test('js/onPointerup simple', async (assert) => {
		...
		const onPointerup1 = actor1.createNode('onPointerup');
		const setObjectPosition1 = actor1.createNode('setObjectPosition');
		const getObjectProperty1 = actor1.createNode('getObjectProperty');
		const negate1 = actor1.createNode('negate');

		onPointerup1.setEventEmitter(CoreEventEmitter.CANVAS);
		setObjectPosition1.setInput(JsConnectionPointType.TRIGGER, onPointerup1);
