...
export function testenginenodesjsSin(qUnit: QUnit) {

qUnit.test('js/sin with float inputs', async (assert) => {
	...
	await RendererUtils.withViewer({cameraNode: perspective_camera1}, async (args) => {
		...
		assert.in_delta(object.position.y, 0.0998, 0.02, 'object moved');
	});
});

qUnit.test('js/sin with vector inputs', async (assert) => {
	const scene = window.scene;
	const perspective_camera1 = window.perspective_camera1;
	...
	setObjectPosition1.setInput(JsConnectionPointType.TRIGGER, onManualTrigger1);
	setObjectPosition1.setInput('position', sin1);
	...
	await RendererUtils.withViewer({cameraNode: perspective_camera1}, async (args) => {
		...
		onManualTrigger1.p.trigger.pressButton();
		await CoreSleep.sleep(100);
		assert.in_delta(object.position.x, 0.0998, 0.02, 'object moved ');
		assert.in_delta(object.position.y, 0.644, 0.05, 'object moved ');
		assert.in_delta(object.position.z, 0.909, 0.05, 'object moved ');
	});
});
