...
export function testenginenodesjsManhattanDistance(qUnit: QUnit) {

qUnit.test('js/manhattanDistance with vector2 inputs', async (assert) => {
	...
	const manhattanDistance = actor1.createNode('manhattanDistance');
	...
	floatToVec3_1.setInput('y', manhattanDistance);

	manhattanDistance.setInput(0, constant1);
	...
	await RendererUtils.withViewer({cameraNode: perspective_camera1}, async (args) => {
		...
		assert.in_delta(object.position.y, 1, 0.01, 'object moved');
	});
});

qUnit.test('js/manhattanDistance with vector3 inputs', async (assert) => {
	const scene = window.scene;
	const perspective_camera1 = window.perspective_camera1;
	...
	manhattanDistance.setInput(0, constant1);
	manhattanDistance.params.get('v1')!.set([0, 1, 0]);
	...
	await RendererUtils.withViewer({cameraNode: perspective_camera1}, async (args) => {
		...
		assert.in_delta(object.position.y, 2, 0.01, 'object moved ');
