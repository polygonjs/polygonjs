...
export function testenginenodesjsOnObjectSwipeleft(qUnit: QUnit) {
	qUnit.test('js/onObjectSwipeleft', async (assert) => {
		...
		const onObjectSwipeleft1 = actor1.createNode('onObjectSwipeleft');
		const setObjectPosition1 = actor1.createNode('setObjectPosition');
		const getObjectProperty1 = actor1.createNode('getObjectProperty');
		const negate1 = actor1.createNode('negate');

		setObjectPosition1.setInput(JsConnectionPointType.TRIGGER, onObjectSwipeleft1);
		...
		await RendererUtils.withViewer({cameraNode: perspective_camera1}, async (args) => {
			...
			triggerPointerdownInMiddle(canvas, new Vector2(2, 0));
			await CoreSleep.sleep(100);
			assert.deepEqual(object.position.toArray(), [0, 0, 0.5], 'pos same');

			triggerPointerupInMiddle(canvas, new Vector2(-2, 0));
			await CoreSleep.sleep(100);
