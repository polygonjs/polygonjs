...
export function testenginenodesjsOnObjectAttributeUpdate(qUnit: QUnit) {
	qUnit.test('js/onObjectAttributeUpdate with number', async (assert) => {
		...
		await RendererUtils.withViewer({cameraNode: perspective_camera1}, async (args) => {
			...
			new ThreejsCoreObject(object, 0).setAttribValue('height', 1);
			await CoreSleep.sleep(100);
			assert.equal(object.position.y, 1, 'object moved to 1');

			new ThreejsCoreObject(object, 0).setAttribValue('height', 0.5);
			await CoreSleep.sleep(100);
	...
	qUnit.test('js/onObjectAttributeUpdate with string', async (assert) => {
		...
		await RendererUtils.withViewer({cameraNode: perspective_camera1}, async (args) => {
			scene.play();
			assert.equal(scene.time(), 0);
			assert.notOk(ThreejsCoreObject.attribValue(object, 'height2'));
			await CoreSleep.sleep(500);
			assert.in_delta(scene.time(), 0.5, 0.25, 'time is 0.5 sec');
			assert.notOk(ThreejsCoreObject.attribValue(object, 'height2'));

			new ThreejsCoreObject(object, 0).setAttribValue('height', 'hahah');
			await CoreSleep.sleep(100);
			assert.equal(ThreejsCoreObject.attribValue(object, 'height2'), 'hahah', 'attrib changed');

			new ThreejsCoreObject(object, 0).setAttribValue('height', 'hu');
			await CoreSleep.sleep(100);
			assert.equal(ThreejsCoreObject.attribValue(object, 'height2'), 'hu', 'attrib changed');
		});
	});
	...
	qUnit.test('js/onObjectAttributeUpdate with vector2', async (assert) => {
		...
		await RendererUtils.withViewer({cameraNode: perspective_camera1}, async (args) => {
			...
			new ThreejsCoreObject(object, 0).setAttribValue('statePos', new Vector2(3, 8));
			await CoreSleep.sleep(100);
			...
			new ThreejsCoreObject(object, 0).setAttribValue('statePos', new Vector2(5, 0.5));
			await CoreSleep.sleep(100);
	...
	qUnit.test('js/onObjectAttributeUpdate with vector3', async (assert) => {
		...
		await RendererUtils.withViewer({cameraNode: perspective_camera1}, async (args) => {
			...
			new ThreejsCoreObject(object, 0).setAttribValue('statePos', new Vector3(3, 1, 2));
			await CoreSleep.sleep(100);
			assert.equal(object.position.x, 3, 'object moved to 1');
			assert.equal(object.position.y, 1, 'object moved to 1');
			assert.equal(object.position.z, 2, 'object moved to 1');

			new ThreejsCoreObject(object, 0).setAttribValue('statePos', new Vector3(5, 0.5, 2.5));
			await CoreSleep.sleep(100);
			assert.equal(object.position.x, 5, 'object moved to 0.5');
			assert.equal(object.position.y, 0.5, 'object moved to 0.5');
	...
	qUnit.test('js/onObjectAttributeUpdate with vector4', async (assert) => {
		...
		await RendererUtils.withViewer({cameraNode: perspective_camera1}, async (args) => {
			...
			new ThreejsCoreObject(object, 0).setAttribValue('statePos', new Vector4(3, 1, 7, 12));
			await CoreSleep.sleep(100);
			assert.equal(object.position.x, 3, 'object moved to 1');
			assert.equal(object.position.y, 1, 'object moved to 1');
			assert.equal(object.position.z, 7, 'object moved to 1');

			new ThreejsCoreObject(object, 0).setAttribValue('statePos', new Vector4(5, 0.5, 8, 41));
			await CoreSleep.sleep(100);
			assert.equal(object.position.x, 5, 'object moved to 0.5');
			assert.equal(object.position.y, 0.5, 'object moved to 0.5');
