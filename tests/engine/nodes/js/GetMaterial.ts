...
export function testenginenodesjsGetMaterial(qUnit: QUnit) {
	qUnit.test('js/GetMaterial', async (assert) => {
		...
		await RendererUtils.withViewer({cameraNode: perspective_camera1}, async (args) => {
			...
			new ThreejsCoreObject(object, 0).setAttribValue('selected', 1);
			...
			assert.equal(
				...
				(await meshBasic1.material()).uuid,
				'mat switched to basic1'
			);

			new ThreejsCoreObject(object, 0).setAttribValue('selected', 0);
			await CoreSleep.sleep(100);
			assert.equal(
				(object.material as Material).uuid,
				(await meshBasic2.material()).uuid,
				'mat switched to basic2'
			);
		});
	});
}
