...
qUnit.test('js/setMaterialUniform', async (assert) => {
	...
	setMaterialUniform1.setInput(JsConnectionPointType.TRIGGER, onManualTrigger1);
	setMaterialUniform1.params.get('uniformName')!.set('myUniform');
	setMaterialUniform1.setUniformType(JsConnectionPointType.VECTOR3);
	setMaterialUniform1.params.get(JsConnectionPointType.VECTOR3)!.set([0.2, 0.3, 0.4]);

	const lerpConstant = actor1.createNode('constant');
	lerpConstant.setJsType(JsConnectionPointType.FLOAT);
	...
	await RendererUtils.withViewer({cameraNode: perspective_camera1}, async (args) => {
		...
		scene.batchUpdates(() => {
			...
			setMaterialUniform1.params.get(JsConnectionPointType.VECTOR3)!.set([0.4, 0.3, 0.8]);
		});
		await CoreSleep.sleep(100);
		onManualTrigger1.p.trigger.pressButton();
		await CoreSleep.sleep(100);
