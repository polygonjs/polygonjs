...
export function testenginenodesjsSetPhysicsRBDCylinderProperty(qUnit: QUnit) {
...
qUnit.test('js/setPhysicsRBDCylinderProperty simple', async (assert) => {
	...
	await RendererUtils.withViewer({cameraNode}, async ({viewer, element}) => {
		...
		scene.batchUpdates(() => {
			...
			multAdd1.setInput(MultAddInput.VALUE, getPhysicsRBDCylinderProperty1, RBDCommonProperty.RADIUS);
