...
export function testenginenodesjsSmootherstep(qUnit: QUnit) {
...
qUnit.test('js/smootherstep with vector inputs', async (assert) => {
	const scene = window.scene;
	const perspective_camera1 = window.perspective_camera1;
	...
	const onManualTrigger1 = actor1.createNode('onManualTrigger');
	const setObjectPosition1 = actor1.createNode('setObjectPosition');
	const smootherstep1 = actor1.createNode('smootherstep');
	const constant1 = actor1.createNode('constant');
	...
	setObjectPosition1.setInput(JsConnectionPointType.TRIGGER, onManualTrigger1);
	setObjectPosition1.setInput('position', smootherstep1);

	smootherstep1.setInput(SmoothstepInput.X, constant1);
	...
	await RendererUtils.withViewer({cameraNode: perspective_camera1}, async (args) => {
		...
		assert.in_delta(object.position.y, 0.8369, 0.05, 'object moved ');
