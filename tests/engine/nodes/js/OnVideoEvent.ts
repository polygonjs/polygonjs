...
QUnit.test('js/OnVideoEvent', async (assert) => {
	...
	setObjectPosition1.setInput(JsConnectionPointType.TRIGGER, onVideoEvent1, VIDEO_EVENT_INDICES.get(VideoEvent.PLAY));
	...
	await RendererUtils.withViewer({cameraNode: perspective_camera1}, async (args) => {
		...
		video1.p.play.set(false);
		await CoreSleep.sleep(100);
		assert.equal(object.position.y, 3, 'move after pause');
		...
		await CoreSleep.sleep(100);
		assert.equal(object.position.y, 5, 'move after vol change');
