...
export function testenginenodesjsOnObjectSwipedown(qUnit: QUnit) {
	qUnit.test('js/onObjectSwipedown', async (assert) => {
		...
		const onObjectSwipedown1 = actor1.createNode('onObjectSwipedown');
		const setObjectPosition1 = actor1.createNode('setObjectPosition');
		const getObjectProperty1 = actor1.createNode('getObjectProperty');
		const negate1 = actor1.createNode('negate');

		setObjectPosition1.setInput(JsConnectionPointType.TRIGGER, onObjectSwipedown1);
