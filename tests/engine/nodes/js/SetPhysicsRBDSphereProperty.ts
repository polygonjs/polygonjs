...
export function testenginenodesjsSetPhysicsRBDSphereProperty(qUnit: QUnit) {
	...
	qUnit.test('js/setPhysicsRBDSphereProperty simple', async (assert) => {
		...
		await RendererUtils.withViewer({cameraNode}, async ({viewer, element}) => {
			...
			scene.batchUpdates(() => {
				...
				multAdd1.setInput(MultAddInput.VALUE, getPhysicsRBDSphereProperty1, RBDCommonProperty.RADIUS);
			...
			//
			scene.batchUpdates(() => {
				setPhysicsRBDSphereProperty1.setInput(
					setPhysicsRBDSphereProperty1.p.radius.name(),
					getObjectAttribute1
				);
			});
			await CoreSleep.sleep(50);
