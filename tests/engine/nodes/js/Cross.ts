...
export function testenginenodesjsCross(qUnit: QUnit) {

qUnit.test('js/cross with vector2 inputs', async (assert) => {
	...
	const cross = actor1.createNode('cross');
	...
	floatToVec3_1.setInput('y', cross);

	cross.setInput(0, constant1);
...
qUnit.test('js/cross with vector3 inputs', async (assert) => {
	const scene = window.scene;
	const perspective_camera1 = window.perspective_camera1;
	...
	setObjectPosition1.setInput(JsConnectionPointType.TRIGGER, onManualTrigger1);
	setObjectPosition1.setInput('position', cross);

	cross.setInput(0, constant1);
	cross.params.get('v1')!.set([0, 1, 0]);
	...
	await RendererUtils.withViewer({cameraNode: perspective_camera1}, async (args) => {
		...
		assert.equal(object.position.y, 0, 'object moved ');
