...
export function testenginenodesjsNearestPosition(qUnit: QUnit) {

qUnit.test('js/NearestPosition', async (assert) => {
	...
	nearestPosition1.setInput(JsConnectionPointType.VECTOR3_ARRAY, elementsToArray1);

	const constants = [constant0, constant1, constant2];
	for (let i = 0; i < constants.length; i++) {
		const constant = constants[i];
		constant.setJsType(JsConnectionPointType.VECTOR3);
