...
export function testenginenodesjsClamp(qUnit: QUnit) {

qUnit.test('js/clamp with float inputs', async (assert) => {
	...
	const clamp = actor1.createNode('clamp');
	...
	floatToVec3_1.setInput('y', clamp);

	clamp.setInput(0, constant1);
...
qUnit.test('js/clamp with vector inputs', async (assert) => {
	const scene = window.scene;
	const perspective_camera1 = window.perspective_camera1;
	...
	setObjectPosition1.setInput(JsConnectionPointType.TRIGGER, onManualTrigger1);
	setObjectPosition1.setInput('position', clamp);

	clamp.setInput(0, constant1);
	clamp.params.get('max')!.set([1, 2, 3]);
	...
	await RendererUtils.withViewer({cameraNode: perspective_camera1}, async (args) => {
		...
		assert.equal(object.position.z, 3, 'object moved ');
	});
});
