...
export function testenginenodesjsTriggerTwoWaySwitch(qUnit: QUnit) {
	qUnit.test('js/triggerTwoWaySwitch', async (assert) => {
		...
		await RendererUtils.withViewer({cameraNode: perspective_camera1}, async (args) => {
			...
			await CoreSleep.sleep(50);
			assert.deepEqual(object.position.toArray(), [0, 1, 0], 'position is correct 1');

			toggleParam.set(1);
			await CoreSleep.sleep(50);
			onManualTrigger1.p.trigger.pressButton();
			await CoreSleep.sleep(50);
			assert.deepEqual(object.position.toArray(), [1, 0, 0], 'position is correct 2');

			toggleParam.set(0);
			await CoreSleep.sleep(50);
			onManualTrigger1.p.trigger.pressButton();
			await CoreSleep.sleep(50);
			assert.deepEqual(object.position.toArray(), [0, 1, 0], 'position is correct 3');

			toggleParam.set(1);
			await CoreSleep.sleep(50);
			onManualTrigger1.p.trigger.pressButton();
			await CoreSleep.sleep(50);
			assert.deepEqual(object.position.toArray(), [1, 0, 0], 'position is correct 4');
		});
	});
}
