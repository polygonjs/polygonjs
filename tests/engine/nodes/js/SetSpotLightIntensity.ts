...
export function testenginenodesjsSetSpotLightIntensity(qUnit: QUnit) {
	qUnit.test('js/setSpotlightIntensity with actor applied to spotlight parent', async (assert) => {
	...
	qUnit.test(
		'js/setSpotlightIntensity with actor applied to spotlight parent and using getObjectChild',
		async (assert) => {
			const scene = window.scene;
			const perspective_camera1 = window.perspective_camera1;
			const geo1 = window.geo1;
			...
			setSpotLightIntensity1.setInput(JsConnectionPointType.TRIGGER, onManualTrigger1);
			setSpotLightIntensity1.setInput(JsConnectionPointType.OBJECT_3D, getObjectChild1);
			setSpotLightIntensity1.p.intensity.set(10);
