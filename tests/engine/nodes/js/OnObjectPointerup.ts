...
export function testenginenodesjsOnObjectPointerup(qUnit: QUnit) {
	qUnit.test('js/onObjectPointerup simple', async (assert) => {
		...
		setObjectPosition1.setInput(JsConnectionPointType.TRIGGER, onObjectPointerup1);
		// setObjectPosition1.p.position.set([0, 0, 1]);
		setObjectPosition1.setInput('position', negate1);
		negate1.setInput(0, getObjectProperty1);
		//
		getIntersectionProperty1.setInput(0, onObjectPointerup1, JsConnectionPointType.INTERSECTION);
		setParam1.setInput(JsConnectionPointType.TRIGGER, onObjectPointerup1);
		setParam1.setInput(
	...
	qUnit.test('js/onObjectPointerup with occluded objects', async (assert) => {
		const scene = window.scene;
		const MAT = window.MAT;
		const perspective_camera1 = window.perspective_camera1;
		...
		onObjectPointerup1.p.blockObjectsBehind.set(false);
		...
		await RendererUtils.withViewer({cameraNode: perspective_camera1}, async (args) => {
			...
			onObjectPointerup1.p.blockObjectsBehind.set(true);
			await CoreSleep.sleep(100);
			triggerPointerupInMiddle(canvas);
			await CoreSleep.sleep(100);
			assert.deepEqual(object1.position.toArray(), [0, 0, -0.5], 'pos set 1');
