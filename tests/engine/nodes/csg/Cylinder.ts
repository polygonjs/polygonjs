...
QUnit.test('csg/cylinder simple', async (assert) => {
	const geo1 = window.geo1;
	geo1.flags.display.set(false); // cancels geo node displayNodeController

	const csgNetwork1 = geo1.createNode('csgNetwork');
	const cylinder1 = csgNetwork1.createNode('cylinder');
	...
	assert.equal(geometry?.getAttribute('position').array.length, 1692);
