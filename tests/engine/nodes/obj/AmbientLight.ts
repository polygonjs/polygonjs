QUnit.test('ambient light simple', async (assert) => {
	...
	assert.equal(ambient_light1.cookController.cooks_count, 0);
	...
	assert.equal(ambient_light1.cookController.cooks_count, 1, 'cooks count should be 1');
...
QUnit.test('ambient light display flag off still cooks', async (assert) => {
	...
	assert.equal(ambient_light1.cookController.cooks_count, 1, 'has cooked');
