...
export function testenginenodescopCanvas(qUnit: QUnit) {
	qUnit.test('cop/canvas simple', async (assert) => {
		...
		await RendererUtils.withViewer({cameraNode: camera}, async (options) => {
			...
			render.p.cameraPath.set(`*${camera.object.name}`);
			...
			assert.in_delta(data[0], 0, 0.02, 'black');
			assert.in_delta(data[1], 0, 0.02, 'black');
			assert.in_delta(data[2], 0, 0.02), 'black';
			assert.in_delta(data[3], 0, 0.02), 'transparent';
