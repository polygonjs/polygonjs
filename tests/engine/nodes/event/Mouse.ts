...
QUnit.test('mouse event nodes simple', async (assert) => {
	...
	await RendererUtils.withViewer({cameraNode: window.perspective_camera1}, async ({viewer, element}) => {
		...
		assert.deepEqual(viewer.eventsController().registeredEventTypes(), ['mousedown', 'mousemove', 'mouseup']);
		...
		await mouse1.compute();
		assert.deepEqual(viewer.eventsController().registeredEventTypes(), ['mousemove', 'mouseup']);

		mouse1.p.mouseup.set(0);
		await mouse1.compute();
		assert.deepEqual(viewer.eventsController().registeredEventTypes(), ['mousemove']);
		...
		await RendererUtils.withViewer({cameraNode: window.perspective_camera1}, async (args2) => {
			assert.deepEqual(args2.viewer.eventsController().registeredEventTypes(), ['mousemove']);
		});
	});
});
