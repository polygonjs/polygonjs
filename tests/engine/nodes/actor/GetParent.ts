...
QUnit.test('actor/GetParent', async (assert) => {
	...
	constant1.setConstantType(ActorConnectionPointType.STRING);
	constant1.p.string.set('*target');

	getObjectProperty1.setInput(0, getParent1);
	getParent1.setInput('mask', constant1);
	getParent1.p.getCurrentObject.set(0);
