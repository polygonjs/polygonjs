...
QUnit.test('actor/length with vector2 inputs', async (assert) => {
	...
	const length = actor1.createNode('length');
	...
	floatToVec3_1.setInput('y', length);
...
QUnit.test('actor/length with vector3 inputs', async (assert) => {
	const scene = window.scene;
	const perspective_camera1 = window.perspective_camera1;
	...
	await RendererUtils.withViewer({cameraNode: perspective_camera1}, async (args) => {
		...
		assert.in_delta(object.position.y, 1.732, 0.01, 'object moved ');
		assert.equal(object.position.z, 0, 'object moved ');
	});
});

QUnit.test('actor/length with vector4 inputs', async (assert) => {
	const scene = window.scene;
	const perspective_camera1 = window.perspective_camera1;
