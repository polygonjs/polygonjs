...
function _createScene(parentNode: GeoObjNode) {
	...
	function create_actor4(parentNode: GeoObjNode) {
		...
		function create_vector3Project1(actor4: ActorSopNode) {
			var vector3Project1 = actor4.createNode('vector3Project');
			vector3Project1.setName('vector3Project1');
			const vector3Project1_nodes: ReturnedNodeDataDict = {};
			vector3Project1.uiData.setPosition(100, 300);
			vector3Project1.p.Vector3.set([0, 0, 0.7]);
			vector3Project1.params.postCreateSpareParams();
			vector3Project1.params.runOnSceneLoadHooks();
			return {node: vector3Project1, children: vector3Project1_nodes};
		}
		...
		actor4_nodes['vector3Project1'] = create_vector3Project1(actor4);
		actor4_nodes['setObjectPosition1'].node.setInput('trigger', actor4_nodes['onTick1'].node, 'trigger');
		actor4_nodes['setObjectPosition1'].node.setInput('position', actor4_nodes['vector3Project1'].node, 'position');
		actor4_nodes['vector3Project1'].node.setInput('Camera', actor4_nodes['getDefaultCamera1'].node, 'camera');
		if (actor4.childrenController) {
			actor4.childrenController.selection.set([actor4_nodes['vector3Project1'].node]);
		}
...
QUnit.test('actor/Vector3Project', async (assert) => {
	...
	await RendererUtils.withViewer({viewer: viewer as ThreejsViewer<PerspectiveCamera>, mount: true}, async (args) => {
		...
		assert.in_delta(object.position.y, 3.0635, 0.01);
