...
vec3 applyMaterialWithoutRefraction(vec3 p, vec3 n, vec3 rayDir, int mat, inout SDFContext sdfContext){
	...
	if(mat == _MAT_RAYMARCHINGBUILDER1_SDFMATERIAL1){
		...
		envMapProps.tint = v_POLY_envMapTint_val;
		envMapProps.intensity = v_POLY_abs1_val;
		envMapProps.roughness = 0.0;
		envMapProps.fresnel = v_POLY_envMapFresnel_val;
		envMapProps.fresnelPower = v_POLY_envMapFresnelPower_val;
