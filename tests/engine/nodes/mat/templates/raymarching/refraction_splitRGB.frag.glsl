...
SDFContext GetDist(vec3 p) {
	...
	SDFContext v_POLY_SDFContext1_SDFContext = SDFContext(v_POLY_SDFGradient1_sdf, 0, _MAT_RAYMARCHINGBUILDER1_SDFMATERIAL1, _MAT_RAYMARCHINGBUILDER1_SDFMATERIAL1, 0.);
...
vec3 applyMaterial(vec3 p, vec3 n, vec3 rayDir, int mat, inout SDFContext sdfContext){
	...
	if(mat == _MAT_RAYMARCHINGBUILDER1_SDFMATERIAL1){
		...
		vec4 refractedDataR = GetRefractedData(p, n, rayDir, ior+offset.x, biasMult, 0.0, 100.0, 3, sdfContext);
		vec4 refractedDataG = GetRefractedData(p, n, rayDir, ior+offset.y, biasMult, 0.0, 100.0, 3, sdfContext);
		vec4 refractedDataB = GetRefractedData(p, n, rayDir, ior+offset.z, biasMult, 0.0, 100.0, 3, sdfContext);
