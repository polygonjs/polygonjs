...
import type {AttribCreateSopNode} from '../../../src/engine/nodes/sop/AttribCreate';
...
export function testengineexpressionsMissingReferences(qUnit: QUnit) {
	...
	qUnit.test(
		'a top node referenced in an expression gets renamed involves updating the expression',
		async (assert) => {
	...
	qUnit.test('string element outside quotes do not generate a warning', async (assert) => {
		const scene = window.scene;
		const geo1 = window.geo1;
		await scene.waitForCooksCompleted();
		...
		attribCreate1.p.string.set(`t\`$F\``);
		...
		const consoleHistory = await checkConsolePrints(async () => {
			await saveAndLoadScene(scene, async (scene2) => {
				...
				const attribCreate2 = scene2.node(attribCreate1.path()) as AttribCreateSopNode;
				await attribCreate2.compute();
				console.log('loaded');
			});
		});
		assert.equal(consoleHistory.log.length, 2, '2 logs');
		assert.equal(consoleHistory.warn.length, 0, 'no warning');
	});
}
