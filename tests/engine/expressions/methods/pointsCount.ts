...
export function testengineexpressionsmethodspointsCount(qUnit: QUnit) {
	...
	qUnit.test('pointsCount: if dependent is deleted, node becomes dirty', async (assert) => {
		...
		assert.equal((box2.p.size.graphPredecessors() || []).length, 0, '0 predecessors');
		assert.equal(box2.p.size.graphAllPredecessors().length, 0, '0 all predecessors');
		assert.equal((box2.p.size.graphPredecessors() || []).length, 0, '0 predecessors');

		await box2.p.size.compute();
		assert.equal((box2.p.size.graphPredecessors() || []).length, 0, '0 predecessors after compute');
		assert.equal(box2.p.size.value, 24);
		assert.equal(
