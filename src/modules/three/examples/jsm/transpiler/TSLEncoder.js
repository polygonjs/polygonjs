...
${this.tab}} )`;
	...
	emitLoop(node) {
		...
		let loopStr = `loop( { start: ${start}, end: ${
			end + nameParam + typeParam + conditionParam + updateParam
		} }, ( { ${name} } ) => {\n\n`;
	...
	emitOverloadingFunction(nodes) {
		...
		return `const ${name} = overloadingFn( [ ${nodes
			.map((node) => node.name + '_' + nodes.indexOf(node))
			.join(', ')} ] );\n`;
	}
...
${this.tab}} );\n\n`;
	...
	emitExtraLine(statement) {
		...
		const isExpression = (st) =>
			st.isFunctionDeclaration !== true && st.isFor !== true && st.isConditional !== true;
