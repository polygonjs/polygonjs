	...
	static updateObject(options: UpdateObjectOptions) {
		...
		if (foundNode && active) {
			...
			for (let object of objects) {
				ThreejsObject.addAttribute(object, CameraAttribute.CSS_RENDERER_NODE_ID, nodeId);
			}
		} else {
			for (let object of objects) {
				ThreejsObject.deleteAttribute(object, CameraAttribute.CSS_RENDERER_NODE_ID);
			}
		}
	}
}
