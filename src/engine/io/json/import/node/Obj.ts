...
export class BaseNodeObjJsonImporter extends NodeJsonImporter {
	...
	from_data_custom(data) {
		...
		if(this._node.has_display_flag()){
			const state = data['display_flag']
			if(state !== undefined){
				this._node.set_display_flag(state)
			}
		}
	}
