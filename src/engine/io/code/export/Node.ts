...
export class NodeCodeExporter {
	...
	create_function_declare(parent_var_name='parent'): string[]{
		...
		this._node.scene().register_node_context_signature(this._node)
	...
	create_function_call(parent_var_name?: string): string{
		if (parent_var_name == null){
			parent_var_name = this._node.parent().visit(CodeExporterVisitor).var_name()
		}
	...
	function_name(): string{
		return "create_" + this.var_name().replace(/[^a-z0-9]/gmi, "_")
	}
	...
	protected add_create(parent_var_name?: string){
		if (parent_var_name == null) {
			const parent_exporter = this._node.parent().visit(CodeExporterVisitor)
			parent_var_name = parent_exporter.var_name();
		}
		const create_node_line = `var ${this.var_name()} = ${parent_var_name}.create_node('${this._node.constructor.type()}')`
		this._lines.push(create_node_line)
	}

	protected add_name(){
		if (this !== this._node.root()) {
	...
	protected add_input() {
		...
		Object.keys(this._node.inputs()).forEach((input_index)=>{
			const input = this._node.input(input_index)
			const input_connections = this._node.input_connections()
			if(input){
				const output_index = input_connections[input_index].output_index()
				const input_exporter = input.visit(CodeExporterVisitor)
				let line
				if(this._node.has_named_inputs()){
					const input_name = this._node.named_inputs()[input_index].name()
					const output_name = input.named_outputs()[output_index].name()
					line = `${this.var_name()}.set_input('${input_name}', ${input_exporter.var_name()}, '${output_name}')`
				} else {
	...
	protected add_position(options: object={}){
		if (this._node.parent()) { // if not root
			let x_offset;
	...
	protected add_params(){
		this._node.param_names().forEach(param_name=>{
			const param = this._node.param(param_name)
			const param_exporter = param.visit(CodeExporterVisitor)
	...
	protected add_input_clonable_state(){
		if(this._node.override_clonable_state_allowed()){
			const override = this._node.override_clonable_state()
			if(override){
	...
	protected add_bypass_flag(){
		if (this._node.has_bypass_flag()) {
			if (this._node.is_bypassed()) {
				this._lines.push(`${this.var_name()}.set_bypass_flag(true)`);
			}
		}
	}
	...
	protected add_selection(){
		if (this._node.selection() != null) {
			for(let node of this._node.selection().nodes()){
				const node_exporter = node.visit(CodeExporterVisitor)
