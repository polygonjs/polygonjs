...
const THREE = {DirectionalLight}

export class DirectionalLightObj extends BaseLightTransformed {

	constructor() {
		super();
	}
	static type() { return 'directional_light'; }

	create_object() {
		const object = new THREE.DirectionalLight();
	...
	update_light_params() {
		...
		if ((object = this.object()) != null) {
			...
			object.shadow.camera.far = this._param_distance;

			this._direction = this._param_t.clone().sub(object.target.position).normalize();
		}
	}
