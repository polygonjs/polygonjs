...
export function AssemblerOwner<TBase extends Constructor>(Base: TBase) {
	return class Mixin extends Base {
		...
		children_context(){ return NodeContext.GL }
		...
		on_create(){
			...
			globals.ui_data().set_position(new THREE.Vector2(-200, 0))
			output.ui_data().set_position(new THREE.Vector2(200, 0))
		}

		async compile_if_required(){
			this._recompiled = false
			if(this.compile_required()){ // && !this._param_locked){
		...
		private async eval_params_and_assign_uniform_values(){
			if( !this.self.scene().loaded() ){return}
		...
		create_spare_parameters(){
			...
			if(spare_params_with_same_name_as_params.length > 0){
				const error_message = `${this.self.full_path()} attempts to create spare params called '${spare_params_with_same_name_as_params.join(', ')}' with same name as params`
