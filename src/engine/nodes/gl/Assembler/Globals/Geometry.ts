...
export class GlobalsGeometryHandler extends GlobalsBaseController {
	...
	handle_globals_node(
		...
		for (let dependency of dependencies) {
			definitions_by_shader_name.get(dependency)!.push(definition);
			body_lines_by_shader_name.get(dependency)!.push(body_line);
		}
	...
	static read_attribute(
		...
		switch (shader_name) {
			...
			case ShaderName.FRAGMENT: {
				...
				// 	fragment: [],
				// };
				definitions_by_shader_name.get(shader_name)!.push(varying_definition);
