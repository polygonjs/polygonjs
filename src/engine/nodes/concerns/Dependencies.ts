...
export function Dependencies<TBase extends Constructor>(Base: TBase) {
	return class Mixin extends Base {
		protected self: BaseNode = (<unknown>this) as BaseNode;

		private _params_referrees_by_graph_node_id: ParamByString;
		...
		scene_successors() {
			return this._find_scene_node_scene_nodes(this.self, METHODS.SUCCESSORS);
		}

		scene_predecessors() {
			return this._find_scene_node_scene_nodes(this.self, METHODS.PREDECESSORS);
		}
		private _find_scene_node_scene_nodes(node: NodeScene, method: METHODS) {
			const params: NodeScene[] = this.self.all_params();
			params.push(this.self);
			...
			scene_nodes = scene_nodes.filter((scene_node) => {
				return scene_node.graph_node_id() != this.self.graph_node_id();
			});
