...
export class BooleanCadNode extends TypedCadNode<BooleanCadParamsConfig> {
	...
	private _createBooleansAllInSequence(
		...
		let shapeObjects = inputCoreGroup0.objectsWithShape();
		if (inputCoreGroup1) {
			shapeObjects = shapeObjects.concat(inputCoreGroup1.objectsWithShape());
		}
		// TODO: shapes with same types should be processed together?
		// Or maybe not if dealing with curves and surfaces
		_createBooleansAllInSequence(oc, operation, shapeObjects, newObjects);
	...
	private _createBooleansOneToOne(
		...
		let shapeObjects0 = inputCoreGroup0.objectsWithShape();
		let shapeObjects1 = inputCoreGroup1.objectsWithShape();
...
function _createBooleansAllInSequence(
	oc: OpenCascadeInstance,
	operation: BooleanCadOperationType,
	shapeObjects: CadCoreObject<CadObjectTypeShape>[],
	newObjects: CadCoreObject<CadObjectType>[]
) {
	let previousShape: TopoDS_Shape | undefined;
	for (let shapeObject of shapeObjects) {
		if (previousShape) {
			const newShape = _booleanOperation(oc, operation, previousShape, shapeObject.object());
			previousShape = newShape;
		} else {
			previousShape = shapeObject.object();
		}
	}
	if (previousShape) {
		const type = cadObjectTypeFromShape(oc, previousShape);
		if (type) {
			newObjects.push(new CadCoreObject(previousShape, type));
		} else {
...
function _createBooleansOneToOne(
	oc: OpenCascadeInstance,
	operation: BooleanCadOperationType,
	shapeObjects0: CadCoreObject<CadObjectTypeShape>[],
	shapeObjects1: CadCoreObject<CadObjectTypeShape>[],
	newObjects: CadCoreObject<CadObjectType>[]
) {
	const minVerticesCount = Math.min(shapeObjects0.length, shapeObjects1.length);
	for (let i = 0; i < minVerticesCount; i++) {
		const shape0 = shapeObjects0[i].object();
		const shape1 = shapeObjects1[i].object();
...
function _createBooleansOneToMany(
	...
	for (let shapeObject0 of shapeObjects0) {
		let previousBooleanShapeResult: TopoDS_Shape = shapeObject0.object();
		...
		if (previousBooleanShapeResult) {
			const type = cadObjectTypeFromShape(oc, previousBooleanShapeResult);
			if (type) {
				newObjects.push(new CadCoreObject(previousBooleanShapeResult, type));
			} else {
