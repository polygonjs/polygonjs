...
export class UIData extends CoreUIData {
	...
	private _color: THREE.Color = new THREE.Color(0.75, 0.75, 0.75)
	private _icon: string = null
	private _layout_vertical: boolean = true
	private _comment: string

	constructor(_node: BaseNode, x: number = 0, y: number = 0) {
		super()
		this._node = _node
		this._position = new THREE.Vector2(x || 0, y || 0)
	}
	...
	set_comment(comment: string) {
		...
		this._node.emit('ui_data_updated', this.to_json())
	}
	comment(): string {
		return this._comment
	}
	set_color(color: THREE.Color) {
		this._color = color
	}
	color() {
		return this._color
	...
	set_position(new_position: THREE.Vector2) {
	...
	translate(offset: THREE.Vector2, snap: boolean = false) {
