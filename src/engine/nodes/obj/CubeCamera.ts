...
const THREE = {CubeCamera, Group, Scene, sRGBEncoding, Object3D, PMREMGenerator}
...
export class CubeCameraObj extends Dirtyable(Transformed(BaseNodeObj)) {
	static type(){return 'cube_camera'}
	...
	create_params() {
		...
		this.within_param_folder('cube_camera', ()=>{
			...
			this.add_param(ParamType.BUTTON, 'render', 0, {
	...
	cook(input_contents){
		...
		this._cube_camera_scene = this._cube_camera_scene || new THREE.Scene()
		...
		if(this._cube_camera){
			const first_camera = this._cube_camera.children[0]
		...
		if(!this._cube_camera || camera_recreate_required){
			this._cube_camera = new THREE.CubeCamera(
				this._param_near,
				this._param_far,
				this._param_resolution,
	...
	render(){
		...
		if(renderer){
			...
			if(dependencies_solved){
				...
				this._cube_camera.renderTarget = render_target
	...
	private object_is_light(object: THREE.Object3D):boolean{
