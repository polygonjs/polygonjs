...
export class CubeCameraObj extends Transformed(BaseObjNode) {
	static type() {
		return 'cube_camera';
	}
	...
	cook(input_contents) {
		...
		if (!this._cube_camera || camera_recreate_required) {
			this._cube_camera = new THREE.CubeCamera(this._param_near, this._param_far, this._param_resolution, {
