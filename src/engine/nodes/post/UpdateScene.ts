...
class UpdateScenePostParamsConfig extends NodeParamsConfig {
	...
	setBlackMaterial = ParamConfig.BOOLEAN(1, {
		...PostParamOptions,
		visibleIf: {reset: 0},
	});
	/** @param reset */
	resetMat = ParamConfig.BUTTON(null, {
		visibleIf: {reset: 0},
		callback: (node: BaseNodeType) => {
...
export class UpdateScenePostNode extends TypedPostProcessNode<UpdateScenePass, UpdateScenePostParamsConfig> {
	...
	protected _createPass(context: TypedPostNodeContext) {
		const pass = new UpdateScenePass({
			...
			setBlackMaterial: isBooleanTrue(this.pv.setBlackMaterial),
		});
		return pass;
	}
	updatePass(pass: UpdateScenePass) {
		pass.reset = isBooleanTrue(this.pv.reset);
		pass.setBlackMaterial = isBooleanTrue(this.pv.setBlackMaterial);
