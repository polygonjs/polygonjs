	...
	return class Mixin extends Base {
		...
		set_expression(string: string) {
			...
			if (this.self.scene().loaded()) {
				this._expression_controller().parse_and_update_dependencies(
					this._expression
				)
			}
		...
		protected async eval_raw_expression(): Promise<any> {
			...
			let value = await this._expression_controller().eval_function(
				this._expression
			)
		...
		_expression_controller() {
			return (this.expression_controller =
				this.expression_controller ||
				new ExpressionController(this.self))
		}
