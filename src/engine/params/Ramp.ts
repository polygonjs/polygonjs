...
export class RampParam extends AsCodeRamp(Single)<RampValue> {
	...
	private _ramp_interpolant: CubicInterpolant
	private _ramp_texture: DataTexture

	static DEFAULT_VALUE = new RampValue('linear', [
		new RampPoint(0, 0),
		new RampPoint(1, 1),
	])
	...
	constructor() {
		...
		this.add_post_dirty_hook(
			this._reset_ramp_interpolant_and_texture.bind(this)
		)
	}
	...
	ramp_texture() {
		return (this._ramp_texture =
			this._ramp_texture || this._create_ramp_texture())
	}
	...
	interpolant() {
		return (this._ramp_interpolant =
			this._ramp_interpolant || this._create_interpolant())
	}
	_create_interpolant() {
