...
export class EmitController {
	...
	emit_allowed(): boolean {
		...
		if (this.param.scene().is_loading()) {
			return false;
		}
		...
		return this.param.scene().emit_allowed();
	}

	block_emit() {
		this._blocked_emit = true;
		if (this.param.is_multiple()) {
			this.param.components().forEach((c: BaseParam) => c.block_emit());
		}
		return true;
	}
	unblock_emit() {
		...
		if (this.param.is_multiple()) {
			this.param.components().forEach((c: BaseParam) => c.unblock_emit());
		}
		return true;
	}
	block_parent_emit() {
	...
	emit_param_updated() {
		if (this.emit_allowed()) {
			this.param.emit('param_updated');

			if (this.param._parent_param != null && this._blocked_parent_emit !== true) {
				this.param._parent_param.emit('param_updated');
			}
		}
		//else
