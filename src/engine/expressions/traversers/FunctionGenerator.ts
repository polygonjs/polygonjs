...
import {
	LiteralConstructsController,
	LiteralConstructMethod,
...
export class FunctionGenerator extends BaseTraverser {
	...
	private function_main_string: string
	...
	public parse_tree(parsed_tree: ParsedTree) {
		...
		if (parsed_tree.error_message == null) {
			try {
				...
				const function_main_string = this.traverse_node(
					parsed_tree.node
				)
				...
				console.warn(
					`error in expression for param ${this.param.full_path()}`
				)
				console.warn(e)
			}
	...
	function_body() {
		if (this.param.expression_for_entities()) {
			return `
			let result;
	...
	protected traverse_CallExpression(node: jsep.CallExpression): string {
		...
		if (method_name) {
			...
			const arguments_joined = `${method_arguments.join(
				ARGUMENTS_SEPARATOR
			)}`
			const direct_function_name =
				DIRECT_EXPRESSION_FUNCTIONS[method_name]
			...
			if (indirect_method) {
				...
				this._create_method_and_dependencies(
					method_name,
					path_argument,
					path_node
				)
	...
	protected traverse_UnaryExpression(node: jsep.UnaryExpression): string {
		if (node.operator === ATTRIBUTE_PREFIX) {
			...
			switch (argument.type) {
				case 'Identifier': {
					const argument_identifier = (<unknown>(
						argument
					)) as jsep.Identifier
				...
				case 'MemberExpression': {
					const argument_member_expression = (<unknown>(
						argument
			...
			if (attribute_name == 'ptnum') {
				...
				this.function_pre_entities_loop_lines.push(
					`const ${var_array} = ${var_attribute}.array`
				)
				if (property) {
	...
	protected traverse_Identifier(node: jsep.Identifier): string {
		...
		if (identifier_first_char == VARIABLE_PREFIX) {
			...
			const direct_constant_name =
				GLOBAL_CONSTANTS[identifier_name_without_dollar_sign]
	...
	protected traverse_Identifier_F(): string {
		this.immutable_dependencies.push(this.param.scene().context())
