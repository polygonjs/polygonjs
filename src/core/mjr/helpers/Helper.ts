...
export class Helper {
	...
	public static shuffleFill<T extends WritableArray<number>>(array: T, rng: PRNG) {
	...
	public static indexBoolArr(array: Uint8Array) {
		let result = 0,
			power = 1;
		for (let i = 0; i < array.length; i++, power *= 2) if (array[i]) result += power;
		return result;
	}

	// TODO: check if bigint is absolutely necessary
	public static indexByteArr(p: Uint8Array, C: bigint) {
		let result = BigInt(0); //0n;
		let power = BigInt(1); //1n;
	...
	public static ords(data: Int32Array, uniques: number[] = []): [Uint8Array, number] {
