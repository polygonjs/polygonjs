...
export class MapNode extends Branch {
	...
	private newgrid: Grid | null = null;
	...
	public override async load(elem: Element, parentSymmetry: Uint8Array, grid: Grid) {
		...
		if (scales.length !== 3) {
			console.error(elem, `scale attribute ${scalestring} should have 3 components separated by space`);
			return false;
		}
		...
		const symmetry =
			SymmetryHelper.getSymmetry(grid.MZ === 1, elem.getAttribute('symmetry'), parentSymmetry) || null;

		const tasks: Promise<Rule[] | null>[] = [];
			...
			tasks.push(
				(async () => {
					...
					for (const r of rule.symmetries(symmetry, grid.MZ === 1)) {
						rules.push(r);
						MapNode.compile(r, grid.MX, grid.MY, grid.MZ, newgrid.MX, newgrid.MY, newgrid.MZ);
					}
					return rules;
				})()
			);
	...
	public override run() {
		...
		if (!this.ip) {
			console.error('this.ip is null');
			return RunState.FAIL;
		}
		if (!grid) {
			console.error('grid is null');
			return RunState.FAIL;
		}
		const {MZ, MY, MX, state} = grid;
		if (!newgrid) {
			console.error('newgrid is null', this);
			return RunState.FAIL;
		}
		...
		for (const rule of this.rules) {
			for (let z = 0; z < MZ; z++)
				for (let y = 0; y < MY; y++)
					for (let x = 0; x < MX; x++)
						if (rule.jit_map_match_kernel && rule.jit_map_match_kernel(state, x, y, z))
							if (rule.jit_map_apply_kernel) {
	...
	private static compile(rule: Rule, MX: number, MY: number, MZ: number, NX: number, NY: number, NZ: number) {
		const {input, output, IO_DIM} = rule;

		const [IMX, IMY, IMZ, OMX, OMY, OMZ] = IO_DIM;

		// jit_map_match_kernel
		if (rule.input.length < OPTIMIZATION_INLINE_LIMIT /*Optimization.inline_limit*/) {
			const code: string[] = [];
						...
						code.push(`
        {
			...
			rule.jit_map_match_kernel = (state: Uint8Array, x: number, y: number, z: number) => {
				for (let dz = 0; dz < IMZ; dz++) {
					for (let dy = 0; dy < IMY; dy++) {
						for (let dx = 0; dx < IMX; dx++) {
							let sx = x + dx;
						...
						code.push(`
        {
			...
			rule.jit_map_apply_kernel = (state: Uint8Array, x: number, y: number, z: number) => {
				for (let dz = 0; dz < OMZ; dz++) {
					for (let dy = 0; dy < OMY; dy++) {
						for (let dx = 0; dx < OMX; dx++) {
							const o = output[dx + dy * OMX + dz * OMX * OMY];
