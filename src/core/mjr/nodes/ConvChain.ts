...
export class ConvChainNode extends Node {
	...
	private substrateColor: number = -1;
	...
	public override async load(elem: Element, _: Uint8Array, grid: Grid) {
		...
		for (let i = 0; i < this.sample.length; i++) this.sample[i] = Number(bitmap[i] == -1);

		const N = (this.N = parseInt(elem.getAttribute('n') || '3') || 3);
		this.steps = parseInt(elem.getAttribute('steps') || '-1') || -1;
		this.temperature = parseFloat(elem.getAttribute('temperature') || '1') || 1;
		const blackStr = elem.getAttribute('black');
		const whiteStr = elem.getAttribute('white');
		const onStr = elem.getAttribute('on');
		this.c0 = blackStr ? grid.values.get(blackStr.charCodeAt(0)) || -1 : -1;
		this.c1 = whiteStr ? grid.values.get(whiteStr.charCodeAt(0)) || -1 : -1;
		this.substrateColor = onStr ? grid.values.get(onStr.charCodeAt(0)) || -1 : -1;
		...
		for (let y = 0; y < SMY; y++)
			for (let x = 0; x < SMX; x++) {
				const p = pattern(N, (dx, dy) => sample[((x + dx) % SMX) + ((y + dy) % SMY) * SMX]);
				...
				for (const q of symmetries) weights[Helper.indexBoolArr(q)] += 1;
			}

		for (let k = 0; k < weights.length; k++) if (weights[k] <= 0) weights[k] = 0.1;
		return true;
	}
	...
	public override run() {
		...
		const {grid, c0, c1, substrate, ip, N, weights, temperature, substrateColor} = this;
		if (!(grid && ip)) {
...
const rotate = (N: number, p: Uint8Array) => pattern(N, (x, y) => p[N - 1 - y + x * N]);
const reflect = (N: number, p: Uint8Array) => pattern(N, (x, y) => p[N - 1 - x + y * N]);
