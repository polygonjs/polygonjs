...
export class PathNode extends Node {
	...
	public finish: number = -1;
	public substrate: number = -1;
	public value: number = -1;
	public inertia: boolean = false;
	public longest: boolean = false;
	public edges: boolean = false;
	public vertices: boolean = false;

	public override async load(elem: Element, _: Uint8Array, grid: Grid) {
		const startSymbols = elem.getAttribute('from');
		if (!startSymbols) return false;
		this.start = grid.wave(startSymbols);
		const colorCharCode = (elem.getAttribute('color') || startSymbols).charCodeAt(0);
		this.value = grid.values.get(colorCharCode) || -1;
		const toStr = elem.getAttribute('to');
		const onStr = elem.getAttribute('on');
		if (!toStr) {
			return false;
		}
		if (!onStr) {
		...
		this.substrate = grid.wave(onStr);
		return true;
	}
	...
	public override run() {
		...
		while (queue.length) {
			...
			for (const [dx, dy, dz] of PathNode.directions(x, y, z, MX, MY, MZ, edges, vertices))
				push(t + 1, x + dx, y + dy, z + dz);
		}

		if (!startPositions.some(([px, py, pz]) => generations[px + py * MX + pz * MX * MY] > 0)) return RunState.FAIL;
		...
		let [dirx, diry, dirz] = this.direction(penx, peny, penz, 0, 0, 0, generations, local);
		penx += dirx;
		peny += diry;
		penz += dirz;
		...
		while (generations[penx + peny * MX + penz * MX * MY]) {
			...
			[dirx, diry, dirz] = this.direction(penx, peny, penz, dirx, diry, dirz, generations, local);
			penx += dirx;
			peny += diry;
			penz += dirz;
		}
	...
	private direction(
		...
		const add = (DX: number, DY: number, DZ: number) => {
			if (generations[x + DX + (y + DY) * MX + (z + DZ) * MX * MY] == g - 1) candidates.push([DX, DY, DZ]);
		};
		...
		if (!vertices && !edges) {
			...
			for (const p of PathNode.directions(x, y, z, MX, MY, MZ, edges, vertices)) add(p[0], p[1], p[2]);
			...
			if (inertia && (dx != 0 || dy != 0 || dz != 0)) {
				...
				for (const [cx, cy, cz] of candidates) {
					...
					const cos =
						(cx * dx + cy * dy + cz * dz) /
						Math.sqrt((cx * cx + cy * cy + cz * cz) * (dx * dx + dy * dy + dz * dz));
	...
	private static directions(
		...
		if (MZ == 1) {
			...
			if (vertices) {
				...
				if (x < MX - 1 && y < MY - 1 && z < MZ - 1) result.push([1, 1, 1]);
			}
		}
