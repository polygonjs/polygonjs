...
export class CorePerformance {
	...
	record(name: string) {
		...
		this._durations_by_name[name] +=
			current_timestamp - this._previous_timestamp
	...
	print_node_cook_data() {
		...
		performance_nodes = lodash_sortBy(
			performance_nodes,
			(performance_node) => -performance_node.cook_time_total()
		)

		const print_objects = performance_nodes.map((performance_node) =>
			performance_node.print_object()
		)

		console.log('--------------- NODES COOK TIME -----------')

		const table_entries = []
		const sorted_print_objects = lodash_sortBy(
			print_objects,
			(print_object) => -print_object['cook_time_total']
		)
