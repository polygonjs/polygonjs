...
export class CoreMath {
	...
	static round(number: number, step_size: number): number {
		...
		const rounded_steps_count =
			number < 0 ? Math.ceil(steps_count) : Math.floor(steps_count)
		console.log(
			'round',
			step_size,
			number,
	...
	static expand_triangle(triangle: Triangle, margin: number): Triangle {
		...
		for (let prop of ['a', 'b', 'c']) {
			...
			triangle[prop as TriangleProp] = mid_point
				.clone()
				.add(delta_n.multiplyScalar(length))
		}
		return triangle
	}
