...
function readWebGLTexture(
	...
	// console.log(renderer.state, renderer.state.buffers);
	// renderer.state.bindXRFramebuffer;
	const renderTarget = renderer.getRenderTarget();
	const previousBuffer: WebGLFramebuffer | null = renderer.properties.get(renderTarget).__webglFramebuffer;
	// const previousBuffer = null;
	...
	// console.log('read', imageData.data[0]);
	canvasContext.putImageData(imageData, 0, 0);
	canvasTexture.needsUpdate = true;
	// } else {
	// console.log('buffer not ready', status, w, h);
	// }
