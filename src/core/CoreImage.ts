...
export class CoreImage {
	...
	static image_from_texture(texture: Texture): Promise<HTMLImageElement> {
		return new Promise((resolve, reject) => {
			if (texture.image) {
	...
	static image_data_to_data_uri(image_data: ImageData): string {
		...
		const context_imaged_data = context.getImageData(
			0,
			0,
			canvas.width,
			canvas.height
	...
	static datauri_from_texture(texture: Texture): string {
		if (texture.image) {
			if (texture.image.data) {
	...
	static make_square(src_img: HTMLImageElement): Promise<HTMLImageElement> {
		return new Promise((resolve, reject) => {
			...
			const margin = is_landscape
				? (src_img.width - size) / 2
				: (src_img.height - size) / 2
