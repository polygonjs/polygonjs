var Module = (() => {
	...
	return async function (Module = {}) {
		...
		Module.setup = function () {
			...
			Module.cylinder = function (height, radiusLow, radiusHigh = -1, circularSegments = 0, center = false) {
			...
			Module.extrude = function (polygons, height, nDivisions = 0, twistDegrees = 0, scaleTop = [1, 1]) {
				if (scaleTop instanceof Array) scaleTop = {x: scaleTop[0], y: scaleTop[1]};
				const polygonsVec = polygons2vec(polygons);
				const result = Module._Extrude(polygonsVec, height, nDivisions, twistDegrees, scaleTop);
		...
		var wasmImports = {
			...
			ha: __embind_register_value_object_field,
			aa: __embind_register_void,
			la: __emval_as,
			qa: __emval_call_method,
			ia: __emval_call_void_method,
			na: __emval_decref,
			ka: __emval_equals,
			P: __emval_get_method_caller,
			ma: __emval_get_property,
			ja: __emval_incref,
			B: __emval_new_cstring,
			ca: __emval_new_object,
			pa: __emval_run_destructors,
			oa: __emval_set_property,
		...
		var ___cxa_free_exception = function () {
			return (___cxa_free_exception = Module['asm']['va']).apply(null, arguments);
		};
		var ___getTypeName = (Module['___getTypeName'] = function () {
			return (___getTypeName = Module['___getTypeName'] = Module['asm']['xa']).apply(null, arguments);
		});
		var __embind_initialize_bindings = (Module['__embind_initialize_bindings'] = function () {
			return (__embind_initialize_bindings = Module['__embind_initialize_bindings'] = Module['asm']['ya']).apply(
				null,
				arguments
			);
		});
		...
		var stackRestore = function () {
			return (stackRestore = Module['asm']['Ba']).apply(null, arguments);
		};
		var ___cxa_can_catch = function () {
			return (___cxa_can_catch = Module['asm']['Ca']).apply(null, arguments);
		};
