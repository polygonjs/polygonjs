		...
		if (name === PTNUM) {
			...
			if (attrib) {
				...
				if (this._core_geometry.is_attrib_indexed(remaped_name)) {
					...
					return this._core_geometry.user_data_attrib(remaped_name)[
						value_index
					]
				} else {
	...
	set_attrib_value(name: string, value: NumericAttribValue) {
		...
		switch (attrib_size) {
			case 1:
				...
				break
			case 2:
				const v2 = value as Vec2
				array[this._index * 2 + 0] = v2.x
				array[this._index * 2 + 1] = v2.y
				break
			case 3:
				var is_color = (value as Col).r != null
				if (is_color) {
					const col = value as Col
			...
			default:
				console.warn(
					`Point.set_attrib_value does not yet allow attrib size ${attrib_size}`
				)
