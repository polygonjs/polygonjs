...
export class CoreObject {
	...
	add_attribute(name: string, value: AttribValue) {
		let data
		if (
			!lodash_isNumber(value) &&
			!lodash_isArray(value) &&
	...
	add_numeric_vertex_attrib(
		name: string,
		size: number,
		default_value: NumericAttribValue
	) {
	...
	human_type(): string {
		return CoreConstant.CONSTRUCTOR_NAMES_BY_CONSTRUCTOR_NAME[
	...
	static clone(src_object: Object3D) {
		...
		CoreObject.parallelTraverse(src_object, new_object, function(
			sourceNode: Object3D,
			clonedNode: Object3D
		) {
		...
		new_object.traverse(function(node) {
			...
			if (mesh_node.geometry) {
				...
				if (mesh_node_geometry.userData) {
					mesh_node_geometry.userData = lodash_cloneDeep(
						src_node_geometry.userData
					)
				}
			}
			if (mesh_node.material) {
				mesh_node.material = src_node.material
				CoreMaterial.apply_custom_materials(
					node,
					mesh_node.material as ShaderMaterialWithCustomMaterials
				)
			...
			const src_node_with_animations = (<unknown>(
				src_node
			)) as Object3DWithAnimations
			if (src_node_with_animations.animations) {
				;(node as Object3DWithAnimations).animations = src_node_with_animations.animations.map(
