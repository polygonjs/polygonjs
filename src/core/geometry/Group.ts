...
export class CoreGroup {
	...
	core_objects() {
		return (this._core_objects =
			this._core_objects || this._create_core_objects())
	}
	...
	geometries() {
		...
		for (let object of this._objects) {
			object.traverse((object) =>
				this.__geometry_from_object.bind(this)(this._geometries, object)
			)
	...
	// 	}
	// }
	static geometry_from_object(object: Object3D): BufferGeometry {
		if (
			((<unknown>object) as Mesh).isMesh ||
			((<unknown>object) as LineSegments).isLine ||
			((<unknown>object) as Points).isPoints
		) {
			return ((<unknown>object) as Mesh).geometry as BufferGeometry
		}
	}
	faces() {
				...
				if (this.has_attrib(old_name)) {
					for (let object of this._objects) {
						object.traverse((child) => {
							const geometry = CoreGroup.geometry_from_object(
								child
							)
							if (geometry) {
								const core_geometry = new CoreGeometry(geometry)
								core_geometry.rename_attribute(
									old_name,
									new_name
								)
							}
