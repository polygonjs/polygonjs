...
export class CoreGeometry {
	...
	bounding_box() {
		return (this._bounding_box =
			this._bounding_box || this._create_bounding_box())
	}
	...
	user_data_attribs() {
		...
		return (this._geometry.userData[key] =
			this._geometry.userData[key] || {})
	}
	...
	add_numeric_attrib(
		name: string,
		size: number = 1,
		default_value: NumericAttribValue = 0
	) {
		const values = []
		...
		if (attribute_added) {
			this._geometry.setAttribute(
				name,
				new Float32BufferAttribute(values, size)
			)
		} else {
	...
	init_position_attribute(points_count: number, default_value: Vector3) {
		...
		return this._geometry.setAttribute(
			'position',
	...
	rename_attribute(old_name: string, new_name: string) {
		if (this.is_attrib_indexed(old_name)) {
			this.user_data_attribs()[new_name] = lodash_clone(
				this.user_data_attribs()[old_name]
			)
		...
		this._geometry.setAttribute(
			new_name,
			new Float32BufferAttribute(old_attrib.array, old_attrib.itemSize)
		)
	...
	points_from_geometry(): CorePoint[] {
		const points = []
		const position_attrib = this._geometry.getAttribute(
			this.position_attrib_name()
		)
	...
	static geometry_from_points(
		...
		if (first_point != null) {
			...
			lodash_each(
				points,
				(point, i) => (new_index_by_old_index[point.index()] = i)
			)

			const indices = this._indices_from_points(
			...
			for (let attribute_name of Object.keys(attributes)) {
				...
				if (is_attrib_indexed) {
					const new_values = lodash_uniq(
						points.map((point) =>
							point.attrib_value(attribute_name)
						)
					)
					const new_index_by_value: NumbersByString = {}
					lodash_each(
						new_values,
						(new_value, i) => (new_index_by_value[new_value] = i)
					)

					geometry_wrapper.user_data_attribs()[
						attribute_name
					] = new_values
					...
					geometry.setAttribute(
						attribute_name,
						new Float32BufferAttribute(new_attrib_indices, 1)
					)
				} else {
					const values = []
				...
				case CoreConstant.OBJECT_TYPE.MESH:
					lodash_each(old_indices, function(old_index, i: number) {
						if (i % 3 === 0) {
							...
							const new_index0 =
								new_index_by_old_index[old_index0]
							const new_index1 =
								new_index_by_old_index[old_index1]
							const new_index2 =
								new_index_by_old_index[old_index2]
							if (
								new_index0 != null &&
								new_index1 != null &&
								new_index2 != null
	...
	static merge_geometries(geometries: BufferGeometry[]) {
		...
		//
		const geometry_wrappers = geometries.map(
			(geometry) => new this(geometry)
		)
		const indexed_attribute_names = geometry_wrappers[0].indexed_attribute_names()
		...
		for (let indexed_attribute_name of indexed_attribute_names) {
			...
			for (let geometry_wrapper of geometry_wrappers) {
				const geometry_points = geometry_wrapper.points()
				for (let point of geometry_points) {
					...
					index_by_values[value] != null
						...
						: (index_by_values[value] = Object.keys(
								index_by_values
						  ).length)
				}
			}
		...
		// 2/3. merge the geos
		//
		const merged_geometry = BufferGeometryUtils.mergeBufferGeometries(
			geometries
		)
		...
		Object.keys(new_values_by_attribute_name).forEach(
			(indexed_attribute_name) => {
				const values =
					new_values_by_attribute_name[indexed_attribute_name]
				merged_geometry_wrapper.set_indexed_attribute_values(
					indexed_attribute_name,
					values
				)
			}
