...
export function Dirtyable<TBase extends Constructor>(Base: TBase) {
	return class DirtyableMixin extends Base {
		...
		set_dirty(original_trigger_graph_node?: DirtyableMixin | null, propagate?: boolean): void {
		...
		set_graph_successors_dirty(original_trigger_graph_node: DirtyableMixin): void {
			const cooker = this.cooker();
			cooker.block();

			const propagate = false;
			this._cached_successors =
				this._cached_successors || this._dirtyable_all_successors(original_trigger_graph_node);
		...
		_dirtyable_all_successors(original_trigger_graph_node: DirtyableMixin): any[] {
			const successors = [];
		...
		set_dirty_allowed(original_trigger_graph_node: DirtyableMixin): boolean {
			throw 'Dirtyable.set_dirty_allowed requires implementation';
		}
		_dirtyable_direct_successors(original_trigger_graph_node: DirtyableMixin): Array<DirtyableMixin> {
