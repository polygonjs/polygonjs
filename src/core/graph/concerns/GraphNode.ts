...
export function GraphNode<TBase extends Constructor>(Base: TBase) {
	return class Mixin extends Base {
		...
		add_graph_input(src: any): boolean {
			return this.graph().connect(src, (<unknown>this) as SceneNodeDirtyable);
		}
		...
		graph_disconnect_predecessors() {
			this.graph().disconnect_predecessors((<unknown>this) as SceneNodeDirtyable);
		}
		graph_disconnect_successors() {
			this.graph().disconnect_successors((<unknown>this) as SceneNodeDirtyable);
		}
