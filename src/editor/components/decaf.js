...
const root_json_children = function(rootState, rootGetters, condition_method){
	...
	json_nodes = lodash_filter(json_nodes, json_node=> lodash_includes(json_root.children, json_node['graph_node_id']));
...
export default component = {
	...
	getters: {
		display_loader(state){ return state.display_loader; },
		...
		current_node_id(state){ return state.current_node_id; },
		...
		current_json_camera_node(state, getters){
			...
			if ((state.current_camera_name == null)) {
				...
				let json_camera = lodash_find(json_cameras, json_camera=> json_camera.name === state.current_camera_name);
	...
	mutations: {
		...
		current_node(state, node){
			if (!node.children_allowed()) { return; }
			return state.current_node_id = node.graph_node_id();
		},
