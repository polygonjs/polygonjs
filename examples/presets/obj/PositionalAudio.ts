...
export function PositionalAudioObjNodePresets() {
	return {
		ambient_mp3: function (node: PositionalAudioObjNode) {
			node.p.url.set(`${DEMO_ASSETS_ROOT_URL}/audio/Thrystero_Diagonal-160.mp3`);
		},
		ambient_ogg: function (node: PositionalAudioObjNode) {
			node.p.url.set(`${DEMO_ASSETS_ROOT_URL}/audio/Thrystero_Diagonal-160.ogg`);
		},
		helicopter_ogg: function (node: PositionalAudioObjNode) {
			node.p.url.set(`${DEMO_ASSETS_ROOT_URL}/audio/497848__amaida1__helicopter-sound.ogg`);
		},
		helicopter_wav: function (node: PositionalAudioObjNode) {
