...
class AttribPromoteSopParamsConfig extends NodeParamsConfig {
  constructor() {
    super(...arguments);
    this.class_from = ParamConfig.INTEGER(DEFAULT.class_from, {
      menu: {
        entries: AttribClassMenuEntries
      }
    });
    this.class_to = ParamConfig.INTEGER(DEFAULT.class_to, {
      menu: {
        entries: AttribClassMenuEntries
      }
    });
    ...
    this.scene.dispatch_controller.on_add_listener(() => {
      this.params.on_params_created("params_label", () => {
        this.params.label.init([this.p.name, this.p.class_from, this.p.class_to], () => {
          if (this.pv.name != "") {
            const from_s = AttribClassMenuEntries.filter((entry) => entry.value == this.pv.class_from)[0].name;
