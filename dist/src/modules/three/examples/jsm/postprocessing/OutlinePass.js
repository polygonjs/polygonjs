...
import {MeshDepthMaterial as MeshDepthMaterial2} from "three/src/materials/MeshDepthMaterial";
...
var OutlinePass = function(resolution, scene, camera, selectedObjects) {
  ...
  this.visibleEdgeColor = new Color2(1, 1, 1);
  this.hiddenEdgeColor = new Color2(0.1, 0.04, 0.02);
  this.edgeGlow = 0;
  ...
  Pass2.call(this);
  this.resolution = resolution !== void 0 ? new Vector22(resolution.x, resolution.y) : new Vector22(256, 256);
  var pars = {minFilter: LinearFilter, magFilter: LinearFilter, format: RGBAFormat};
  var resx = Math.round(this.resolution.x / this.downSampleRatio);
  var resy = Math.round(this.resolution.y / this.downSampleRatio);
  this.maskBufferMaterial = new MeshBasicMaterial2({color: 16777215});
  this.maskBufferMaterial.side = DoubleSide;
  this.renderTargetMaskBuffer = new WebGLRenderTarget2(this.resolution.x, this.resolution.y, pars);
  this.renderTargetMaskBuffer.texture.name = "OutlinePass.mask";
  this.renderTargetMaskBuffer.texture.generateMipmaps = false;
  this.depthMaterial = new MeshDepthMaterial2();
  ...
  this.renderTargetDepthBuffer = new WebGLRenderTarget2(this.resolution.x, this.resolution.y, pars);
  this.renderTargetDepthBuffer.texture.name = "OutlinePass.depth";
  this.renderTargetDepthBuffer.texture.generateMipmaps = false;
  this.renderTargetMaskDownSampleBuffer = new WebGLRenderTarget2(resx, resy, pars);
  this.renderTargetMaskDownSampleBuffer.texture.name = "OutlinePass.depthDownSample";
  this.renderTargetMaskDownSampleBuffer.texture.generateMipmaps = false;
  this.renderTargetBlurBuffer1 = new WebGLRenderTarget2(resx, resy, pars);
  this.renderTargetBlurBuffer1.texture.name = "OutlinePass.blur1";
  this.renderTargetBlurBuffer1.texture.generateMipmaps = false;
  this.renderTargetBlurBuffer2 = new WebGLRenderTarget2(Math.round(resx / 2), Math.round(resy / 2), pars);
  this.renderTargetBlurBuffer2.texture.name = "OutlinePass.blur2";
  this.renderTargetBlurBuffer2.texture.generateMipmaps = false;
  this.edgeDetectionMaterial = this.getEdgeDetectionMaterial();
  this.renderTargetEdgeBuffer1 = new WebGLRenderTarget2(resx, resy, pars);
  this.renderTargetEdgeBuffer1.texture.name = "OutlinePass.edge1";
  this.renderTargetEdgeBuffer1.texture.generateMipmaps = false;
  this.renderTargetEdgeBuffer2 = new WebGLRenderTarget2(Math.round(resx / 2), Math.round(resy / 2), pars);
  ...
  this.oldClearColor = new Color2();
  this.oldClearAlpha = 1;
  this.fsQuad = new Pass2.FullScreenQuad(null);
  this.tempPulseColor1 = new Color2();
  this.tempPulseColor2 = new Color2();
  this.textureMatrix = new Matrix42();
  function replaceDepthToViewZ(string, camera2) {
    var type = camera2.isPerspectiveCamera ? "perspective" : "orthographic";
    return string.replace(/DEPTH_TO_VIEW_Z/g, type + "DepthToViewZ");
  }
};
OutlinePass.prototype = Object.assign(Object.create(Pass2.prototype), {
  ...
  changeVisibilityOfNonSelectedObjects: function(bVisible) {
    ...
    function VisibilityChangeCallBack(object) {
      if (object.isMesh || object.isSprite) {
        var bFound = false;
        for (var i2 = 0; i2 < selectedMeshes.length; i2++) {
          var selectedObjectId = selectedMeshes[i2].id;
  ...
  getPrepareMaskMaterial: function() {
    return new ShaderMaterial2({
      uniforms: {
        ...
        cameraNearFar: {value: new Vector22(0.5, 0.5)},
  ...
  getEdgeDetectionMaterial: function() {
    return new ShaderMaterial2({
      uniforms: {
        ...
        texSize: {value: new Vector22(0.5, 0.5)},
        visibleEdgeColor: {value: new Vector32(1, 1, 1)},
        hiddenEdgeColor: {value: new Vector32(1, 1, 1)}
      },
  ...
  getSeperableBlurMaterial: function(maxRadius) {
    return new ShaderMaterial2({
      ...
      uniforms: {
        ...
        texSize: {value: new Vector22(0.5, 0.5)},
        direction: {value: new Vector22(0.5, 0.5)},
...
OutlinePass.BlurDirectionX = new Vector22(1, 0);
OutlinePass.BlurDirectionY = new Vector22(0, 1);
export {OutlinePass};
