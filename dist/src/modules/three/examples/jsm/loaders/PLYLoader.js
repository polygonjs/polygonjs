...
PLYLoader.prototype = Object.assign(Object.create(Loader2.prototype), {
  ...
  parse: function(data) {
    function parseHeader(data2) {
      ...
      for (var i = 0; i < lines.length; i++) {
        ...
        switch (lineType) {
          case "format":
            header2.format = lineValues[0];
            header2.version = lineValues[1];
            break;
          case "comment":
            header2.comments.push(line);
            break;
          case "element":
            if (currentElement !== void 0) {
              header2.elements.push(currentElement);
            }
            currentElement = {};
    ...
    function parseASCII(data2, header2) {
      var buffer = {
        indices: [],
        vertices: [],
        normals: [],
      ...
      for (var i = 0; i < lines.length; i++) {
        ...
        if (currentElementCount >= header2.elements[currentElement].count) {
          currentElement++;
          currentElementCount = 0;
        }
        var element = parseASCIIElement(header2.elements[currentElement].properties, line);
        handleElement(buffer, header2.elements[currentElement].name, element);
        currentElementCount++;
      }
    ...
    function parseBinary(data2, header2) {
      var buffer = {
        indices: [],
        vertices: [],
        normals: [],
      ...
      var little_endian = header2.format === "binary_little_endian";
      var body = new DataView(data2, header2.headerLength);
      var result, loc = 0;
      for (var currentElement = 0; currentElement < header2.elements.length; currentElement++) {
        for (var currentElementCount = 0; currentElementCount < header2.elements[currentElement].count; currentElementCount++) {
          result = binaryReadElement(body, loc, header2.elements[currentElement].properties, little_endian);
          loc += result[1];
    ...
    if (data instanceof ArrayBuffer) {
      var text = LoaderUtils2.decodeText(new Uint8Array(data));
